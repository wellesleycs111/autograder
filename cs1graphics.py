...
class Point(object):
    ...
    def __eq__(self, other):
        """Is this point equal to other?"""
        #TODO: is this safe, or should we implement as equals?
        return isinstance(other, Point) and self.getX()==other.getX() and self.getY()==other.getY()

    def __ne__(self, other):
        """Is this point not equal to other?"""
...
class Shape(Drawable):
    ...
    def equals(self, other):
        """is this shape equal to the other?"""
        selfprops = self._getProperties()
        otherprops = other._getProperties()
        for p in ['border color', 'border width', 'dash']:
            if p not in selfprops or p not in otherprops:
                return False
            if selfprops[p] != otherprops[p]:
                return False
        return True
...
class FillableShape(Shape):
    ...
    def equals(self, other):
        """is this fillable shape equal to the other?"""
        selfprops = self._getProperties()
        otherprops = other._getProperties()
        for p in ['border color', 'border width', 'dash', 'fill color']:
            if p not in selfprops or p not in otherprops:
                return False
            if selfprops[p] != otherprops[p]:
                return False
...
class Layer(Drawable, _GraphicsContainer):
    ...
    def add(self, drawable):
        ...
        #if drawable in self._contents:  # SR: temporarily getting rid of this because we now have shallow equality testing which messes things up
    ...
    def equals(self, other):
        """Is this layer equal to other?"""
        # TODO: we're not checking the hierarchy for now
        selfset = set(self._contents)
        otherset = set(other._contents)
        foundself = set()
        for selfobj in selfset:
            for otherobj in otherset:
                if selfobj.equals(otherobj):
                    otherset.remove(otherobj)
                    foundself.add(selfobj)
                    break
        return foundself == selfset and not otherset
...
class Path(Shape):
    ...
    def equals(self, other):
        """is this path equal to the other?"""
        selfprops = self._getProperties()
        otherprops = other._getProperties()
        for p in ['arrows', 'border color', 'border width', 'dash', 'points']:
            if p not in selfprops or p not in otherprops:
                return False
            if selfprops[p] != otherprops[p]:
                return False
...
class Polygon(Path,FillableShape):
    ...
    def equals(self, other):
        """is this polygon equal to the other?"""
        selfprops = self._getProperties()
        otherprops = other._getProperties()
        for p in ['border color', 'border width', 'dash', 'fill color', 'points']:
            if p not in selfprops or p not in otherprops:
                return False
            if selfprops[p] != otherprops[p]:
                return False
